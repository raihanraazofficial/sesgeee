    ...
    people: [
      ...
      },
      {
        ...
        social_links: {
          ...
          web_of_science: 'https://www.webofscience.com/wos/author/rid/example3',
        }
      }
    ],
    publications: [
  ...
  const fetchData = useCallback(async (type, params = {}) => {
    try {
      ...
      try {
        ...
        // If Firestore returns empty data, use mock data fallback (except for news, events)
        if (data.length === 0 && type !== 'news' && type !== 'events') {
        ...
        // Don't use mock data for news, and events - return empty array instead
        if (type === 'news' || type === 'events') {
      ...
      // Don't use mock data for people, news, and events - return empty array instead
      if (type === 'people' || type === 'news' || type === 'events') {
