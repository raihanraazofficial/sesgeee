  ...
  const fetchData = useCallback(async (type, params = {}) => {
    try {
      ...
      try {
        ...
        // If Firestore returns empty data, use mock data fallback (except for people and publications)
        if (data.length === 0 && type !== 'people' && type !== 'publications') {
        ...
        // Don't use mock data for people - return empty array instead
        if (type === 'people') {
          console.log(`[DataContext] Firestore failed for people, returning empty array instead of mock data`);
          dispatch({
            type: 'SET_DATA',
            payload: { type, data: [] },
          });
      ...
      // Don't use mock data for people - return empty array instead
      if (type === 'people') {
        console.log(`[DataContext] General error for people, returning empty array instead of mock data`);
        dispatch({
          type: 'SET_DATA',
          payload: { type, data: [] },
        });
